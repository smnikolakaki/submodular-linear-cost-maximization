"""
This class implements the scaled single-threshold max value greedy algorithm
...
class ScaledSingleThresholdMaxValGreedy(object):
    """
    ...
    def __init__(self, config, submodular_func, cost_func, E, k):
        ...
        self.E = E
        self.k = k
        self.epsilon = self.config['algorithms']['scaled_single_threshold_max_val_greedy_config']['epsilon']
    ...
    def run(self):
        ...
        # Find the maximum value of all singletons - First pass
        for i in range(0, len(self.E)):
            e_i = i
            m = max(m, self.calc_scaled_objective(set({e_i}))) 

        # Create the set of thresholds
        O = self.get_set_of_thresholds(m)
        # Create empty sets Sv 
        S = self.update_set_keys(S,O)
